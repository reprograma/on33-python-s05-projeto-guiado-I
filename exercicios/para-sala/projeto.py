# import sys
# sys.path.append('para-sala') # essa pasta sys.path.append (direciona a pasta que será buscada)

import print_reprograma as pr
from datetime import datetime #importando a função datime do módulo datatime (do python)
...
def imprime_compra_fechada(compra, total, desconto):
    ...
    if desconto > 0:
        ...
        pr.imprimir('-R$',str(round(desconto, 2)), tamanho=12, alinhar='fim',cor_texto='verde negrito')
...
def menu():
    opcao = '' # é da biblioteca e imprime o que o usuário digita
    erro = ''
    tela = ''
    compra = []
    compras = []
    while opcao != 'q':
        ...
        if opcao == 'h':
            ...
            compras.append({'itens': compra, 'total': total, 'data': datetime.now(), 'desconto': desconto}) #datatime.nowm mostra a hora da compra
            compra = []
            tela = ''
        else:
            try:
