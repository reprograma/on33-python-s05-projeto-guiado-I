...
def imprime_compra_fechada(compra, total, preco_desconto):  # Esta função recebe a compra e o valor total como entrada
    ...
    # Imprimimos o valor do desconto
    pr.imprimir('Desconto', tamanho=107, alinhar='fim', end='|')  # Imprimimos o desconto calculado
    pr.imprimir('R$', str(round(preco_desconto, 2)), tamanho=12, alinhar='fim')
...
def menu():  # Esta função não recebe nenhuma entrada
    ...
    while opcao != 'q':  # Enquanto a opção do usuário for diferente de 'q' (sair)
        ...
        if tela == '':  # Se a tela atual for vazia (tela inicial)
            ...
            imprime_compra_fechada(compra, total, preco_desconto)  # Chamamos a função 'imprime_compra_fechada' para imprimir a compra fechada
        ...
        if opcao == 'h':  # Se a opção do usuário for 'h' (ajuda)
            ...
            total, preco_desconto = calcula_total_desconto(compra)  # Chamamos a função 'calcula_total_desconto' para calcular o total da compra
...
def calcula_total_desconto(compra):  # Esta função recebe a compra como entrada
    ...
    for i, produto_aux in enumerate(compra):
        codigo_auxiliar = produto_aux['codigo']
        for j in range(i + 1, len(compra)):
            produto = compra[j]
            if codigo_auxiliar == produto['codigo']:
                preco_desconto_aux = (produto['valor'] * produto['quantidade']) / 2
                
                preco_desconto += preco_desconto_aux
    
    total -= preco_desconto
        
    if total > 100:
        preco_desconto = total*(10/100)
        total = total - preco_desconto

    
    # ...
    # Retornamos o total da compra com desconto
    return total, preco_desconto
