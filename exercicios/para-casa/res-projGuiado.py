...
def menu():
    ...
    while opcao != 'q':
        ...
        if opcao == 'h':
            ...
            total, desconto = calcula_total_desconto(compra) 
            tela = 'fechar'
        elif opcao == 'e': # exclui as compras do carrinho
            tela = 'encerar'
        elif 'p' in opcao:
            ...
            produto = produto_codigo(int(opcao))
            if produto:
                ...
                tela = 'compra'
        elif opcao.startswith('x') and opcao[1:].isdigit():
            quantidade = int(opcao[1:])
            if len(compra) > 0:
                compra[-1]['quantidade'] = quantidade
                tela = 'compra'
            else:
                erro = 'Nenhum produto na compra para alterar a quantidade!'
        elif opcao == 'p':
            if compra:
                compras.append({'itens': compra, 'total': total, 'data': datetime.now()})
                compra = [] 
                total = 0 
                erro = 'Compra concluída com sucesso!'
                tela = ''
            else:
                erro = 'Nenhuma compra para pagar!'
                tela = 'compra'
